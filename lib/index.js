...
class BatchLoader {
    constructor(batchLoaderFn, options) {
        const {
            batchFn,
            maxSize,
            cacheTime,
            ...
            parallel, // feature
            getKey,
        } = parseOptions(batchLoaderFn, options)
    ...
    async run() {
        ...
        if (
            ...
            error = new Error(MSG_BAD_RESULT)
        }
        ...
        keys.forEach((key, index) => {
            ...
            this[PROP_WAIT_BATCH].delete(key)
    ...
    loadMany(arr) {
        if (!Array.isArray(arr) || arr.length === 0) throw new TypeError(MSG_LOAD_MANY_METHOD)
    ...
    resolve(key, result) {
        return promiseFns(this, 'resolve', key, result)
    }

    reject(key, error) {
        return promiseFns(this, 'reject', key, error)
    }
}
